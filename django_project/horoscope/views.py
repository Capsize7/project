...
def index(request):
    ...
    for sign in zodiac_dict:
        path_sign = reverse('horoscope-name', args=(sign,))
        li_elements += f'<li><a href={path_sign}>{sign}</a></li>'
    response = f'<ol>{li_elements}</li>'
    return HttpResponse(response)


def get_zodiac_by_number(request, zodiac):
    name_zodiac = list(zodiac_dict)[zodiac - 1]
